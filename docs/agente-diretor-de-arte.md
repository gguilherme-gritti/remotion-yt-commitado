...
src/data/projects/{videoId}/
  ...
  assets/                     # sketches; src = só o nome do arquivo
```
...
```json
{
  "type": "image",
  "src": "monitor_hacker.jpg",
  "imageIdea": "Uma placa de alerta de metal enferrujada, vista de frente.",
  "startAtFrame": 0,
  "animation": "pop_in",
  "position": "center",
  "size": "medium"
...
- `src`: só o filename em `assets/`.
- `imageIdea`: obrigatório. Descreva **um objeto único** (substantivo concreto, 1:1), que faça sentido com o roteiro e com os outros elementos da cena. Não reuse o mesmo objeto em cenas diferentes do ato. Não descreva composição, câmera nem estilo — só o objeto.
...
5. `src` = filename. `imageIdea` = um objeto único, concreto, alinhado ao roteiro e aos outros da cena.
6. Personagem: só as 10 chaves de `pose`, sem extensão.
7. `startAtFrame` = ordem sequencial (`0`, `10`, `20`…). O motor recalcula respiro e cadência.
8. `comic_grid` sem `panel`, `timeline` sem `step`, `nested_zoom` sem `isContainer`/`nested`, `spotlight` sem `isTarget` = JSON inválido.
9. Saída: `scenes.json` + um `layouts/{id}.json` por cena. JSON válido, sem comentários.
10. O input é um **ato**, não o vídeo inteiro. Próximos inputs são atos do mesmo vídeo.
